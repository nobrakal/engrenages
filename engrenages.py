...
def sendTimedMessage(msg, socket_list):
	...
	for sock in socket_list[1:]:
		...
		sock.send(pickle.dumps((str(time)+self.msg))) # Envoi le temps afin d'éviter les boucles d'envoi infinies (=id du message), plus le message
...
class Client():
	...
	def ConnectNewServer(self, ip, port=port_client):
		self.ip = ip
		try:
			self.s.connect((self.ip,port_client))# On se connecte au nouveau serveur.
